...
class CrappyJavaPluginTest {
    ...
    void registers_extension_with_conventions_and_task() {
        ...
        CrappyJavaExtension extension = project.getExtensions().getByType(CrappyJavaExtension.class);
        ...
        assertThat(task.getOutputFile().get().getAsFile())
                .isEqualTo(project.getLayout().getBuildDirectory().file("reports/crappy-java/crap.txt").get().getAsFile());
    ...
    private static CrapTask crapTask(Project project) {
        return project.getTasks().named(CrappyJavaPlugin.TASK_NAME, CrapTask.class).get();
    }
}
