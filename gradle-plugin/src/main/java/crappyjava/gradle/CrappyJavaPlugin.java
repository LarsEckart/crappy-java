...
/**
 * Registers the {@code crap} task and the {@code crappyJava} extension. When the {@code jacoco} plugin
 * is applied, {@code jacocoTestReport} is told to emit XML and {@code crap} consumes it.
 */
public class CrappyJavaPlugin implements Plugin<Project> {

    public static final String EXTENSION_NAME = "crappyJava";
    public static final String TASK_NAME = "crap";
    static final String JACOCO_REPORT_TASK = "jacocoTestReport";

    @Override
    public void apply(Project project) {
        CrappyJavaExtension extension = project.getExtensions().create(EXTENSION_NAME, CrappyJavaExtension.class);
        ...
        TaskProvider<CrapTask> crap = project.getTasks().register(TASK_NAME, CrapTask.class, task -> {
            ...
            task.getOutputFile().convention(project.getLayout().getBuildDirectory().file("reports/crappy-java/crap.txt"));
